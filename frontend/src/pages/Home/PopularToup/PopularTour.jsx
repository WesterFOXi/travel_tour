...
const PopularTour = () => {
   ...
   useEffect(() => {
      setArrCard(arr_cards_cour.filter(el => el.id > target.left && el.id <= target.right))
   }, [target])
      ...
      <div className={cls.popular_tour}>
         <div className={cls.block_text_and_arrow}>
            ...
            <div className={cls.button_arrow}>
               <Button className={target.left === 0 ? cls.disabled : cls.active} onClick={() => countLeft()}><MdKeyboardArrowLeft /></Button>
               <Button className={target.right >= arr_cards_cour.length ? cls.disabled : cls.active} onClick={() => countRight()}><MdKeyboardArrowRight /></Button>
            </div>
         </div>

         <TransitionGroup className={cls.cards_conteiner}>
